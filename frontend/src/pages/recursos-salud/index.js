...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("recursos-salud/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_recursos_salud: row.id_recursos_salud
                });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("recursos-salud/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_recursos_salud: row.id_recursos_salud
                });
    ...
    const tableMain = useMemo(() => ({
        ...
        headers: [
            ...
            { id: 'totProfesionales', label: 'Profesionales', dataType: DataType.NUMBER, sortable: true },
            { id: 'totTecnicos', label: 'Técnicos', dataType: DataType.NUMBER, sortable: true },
            { id: 'totAuxiliares', label: 'Auxiliares', dataType: DataType.NUMBER, sortable: true },
            { id: 'totOtros', label: 'Otros', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("recursos-salud/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
        ...
        rowActions: (row) => {
            ...
            acciones.push({
                label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                    setIdReferenciaVerLogModal(row.id_recursos_salud);
                    setReferenciaVerLogModal(Formatter.log_referencia.REC_SALUD.id);
                }
            });
    ...
    return (
        <Layout nombrePagina="Recursos de Salud">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verRecursosSalud"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
