...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("produccion-mensual/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_produccion_mensual: row.id_produccion_mensual
                });
    ...
    const onVerSolicitud = useCallback(async (row) => {
        loadingModal.setOpen(true);
        globalsContext.setIdEdit(row.id_produccion_mensual);
        navigate("/produccion-mensual/ver-solicitud");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [globalsContext, navigate]);

    const newInforme = useCallback(async (row) => {
        globalsContext.setIdEdit(false);
        navigate("/produccion-mensual/nuevo");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [/*globalsContext, */navigate]);

    const onCorregir = useCallback(async (row) => {
        globalsContext.setIdEdit(row.id_produccion_mensual);
        navigate("/produccion-mensual/editar");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [usuario.id_usuario]);
        ...
        /*if(usuario.admin==0 && row.id_usuario_carga == usuario.id_usuario){
            ...
            if (response.error === "") {
                ...
                if(response.data.length > 1){
                    ...
                    globalsContext.setIdEdit(row.id_produccion_mensual);
                    navigate("/produccion-mensual/aprobar-rechazar");
                }
            }
            else
                enqueueSnackbar(response.error, { variant: "error" });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("produccion-mensual/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_produccion_mensual: row.id_produccion_mensual
                });
    ...
    const tableMain = useMemo(() => ({
        ...
        headers: [
            ...
            { id: 'totDiagnos', label: 'Diag. por Imágen', dataType: DataType.NUMBER, sortable: true },
            { id: 'totOtras', label: 'Estudios Especiales', dataType: DataType.NUMBER, sortable: true },
            { id: 'totRehab', label: 'Rehabilitaciones', dataType: DataType.NUMBER, sortable: true },
            { id: 'totCirugias', label: 'Cirugías', dataType: DataType.NUMBER, sortable: true },
            { id: 'totPartos', label: 'Partos', dataType: DataType.NUMBER, sortable: true },
            { id: 'totProdParto', label: 'Prod. del Parto', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("produccion-mensual/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
        ...
        rowActions: (row) => {
            ...
            acciones.push({
                label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                    setIdReferenciaVerLogModal(row.id_produccion_mensual);
                    setReferenciaVerLogModal(Formatter.log_referencia.PROD_MENSUAL.id);
                }
            });
    ...
    return (
        <Layout nombrePagina="Producción Mensual">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verProduccionMensual"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
