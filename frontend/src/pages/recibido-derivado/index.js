...
import { amber, blue, cyan, deepPurple, pink } from '@mui/material/colors';
...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("recibido-derivado/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_recibido_derivado: row.id_recibido_derivado
                });
    ...
    const newInforme = useCallback(async (row) => {
        ...
        navigate("/recibido-derivado/nuevo");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [/*globalsContext, */navigate]);

    const onCorregir = useCallback(async (row) => {
        globalsContext.setIdEdit(row.id_recibido_derivado);
        navigate("/recibido-derivado/editar");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [usuario.id_usuario]);

    const onAvanzar = useCallback(async (row) => {
        alertModal.showModal(
            "Enviar a supervisión",
            "La planilla será enviada a supervisión para su revisión y aprobación. ¿Deseas continuar?",
            async function () {
                loadingModal.setOpen(true);

                const response = await postData("recibido-derivado/avanzar.php", {
                    id_usuario: usuario.id_usuario,
                    id_recibido_derivado: row.id_recibido_derivado
                });

                if (response.error === "") {
                    enqueueSnackbar("Informe enviado a supervisión ok", { variant: "success" });
                    setTableMainNeedUpdate(Date.now());
                }
                else
                    enqueueSnackbar(response.error, { variant: "error" });
        ...
        /*if (usuario.admin == 0 && row.id_usuario_carga == usuario.id_usuario) {
            ...
            if (response.error === "") {
                ...
                if (response.data.length > 1) {
                    ...
                    globalsContext.setIdEdit(row.id_recibido_derivado);
                    navigate("/recibido-derivado/aprobar-rechazar");
                }
            }
            else
                enqueueSnackbar(response.error, { variant: "error" });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("recibido-derivado/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_recibido_derivado: row.id_recibido_derivado
                });
    ...
    const tableMain = useMemo(() => ({
        title: "Pacientes Recibidos y Derivados",
        idName: "id_recibido_derivado",
        defaultOrder: "desc",
        headers: [
            ...
            { id: 'totalRec', label: 'Recibidos', dataType: DataType.NUMBER, sortable: true },
            { id: 'totalDer', label: 'Derivados', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("recibido-derivado/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
        ...
        rowActions: (row) => {
            ...
            acciones.push({
                label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                    setIdReferenciaVerLogModal(row.id_recibido_derivado);
                    setReferenciaVerLogModal(Formatter.log_referencia.RECIB_DERIV.id);
                }
            });

            if (row.responsable == 1 && (row.estado == 1 || row.estado == 2)) {
                if (usuario.carga == 1 || usuario.admin == 1) {
                    acciones.push({ label: "Modificar planilla", icon: <ListAltRoundedIcon sx={{ color: blue[500] }} />, action: () => onCorregir(row) });
                    acciones.push({ label: "Enviar a supervisión", icon: <OutputRoundedIcon sx={{ color: blue[500] }} />, action: () => onAvanzar(row) });
                }
            }
    ...
    return (
        <Layout nombrePagina="Pacientes Recibidos y Derivados">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verRecibidoDerivado"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
