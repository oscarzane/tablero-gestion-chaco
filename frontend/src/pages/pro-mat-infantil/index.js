...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("pro-mat-infantil/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_pro_mat_infantil: row.id_pro_mat_infantil
                });
    ...
    const onVerSolicitud = useCallback(async (row) => {
        loadingModal.setOpen(true);
        globalsContext.setIdEdit(row.id_pro_mat_infantil);
        navigate("/pro-mat-infantil/ver-solicitud");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [globalsContext, navigate]);

    const newInforme = useCallback(async (row) => {
        globalsContext.setIdEdit(false);
        navigate("/pro-mat-infantil/nuevo");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [/*globalsContext, */navigate]);

    const onCorregir = useCallback(async (row) => {
        globalsContext.setIdEdit(row.id_pro_mat_infantil);
        navigate("/pro-mat-infantil/editar");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [usuario.id_usuario]);
        ...
        /*if(usuario.admin==0 && row.id_usuario_carga == usuario.id_usuario){
            ...
            if (response.error === "") {
                ...
                if(response.data.length > 1){
                    ...
                    globalsContext.setIdEdit(row.id_pro_mat_infantil);
                    navigate("/pro-mat-infantil/aprobar-rechazar");
                }
            }
            else
                enqueueSnackbar(response.error, { variant: "error" });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("pro-mat-infantil/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_pro_mat_infantil: row.id_pro_mat_infantil
                });
    ...
    const tableMain = useMemo(() => ({
        ...
        headers: [
            ...
            { id: 'men1Total', label: 'Total < 1 año', dataType: DataType.NUMBER, sortable: true },
            { id: 'de1Total', label: 'Total 1 año', dataType: DataType.NUMBER, sortable: true },
            { id: 'de2a5Total', label: 'Total 2 a 5 años', dataType: DataType.NUMBER, sortable: true },
            { id: 'embTotal', label: 'Total Embarazadas', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("pro-mat-infantil/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
        ...
        rowActions: (row) => {
            ...
            acciones.push({
                label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                    setIdReferenciaVerLogModal(row.id_pro_mat_infantil);
                    setReferenciaVerLogModal(Formatter.log_referencia.P_M_INFANTIL.id);
                }
            });
    ...
    return (
        <Layout nombrePagina="Programa Materno Infantil">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verMaterno"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
