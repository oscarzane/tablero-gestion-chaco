...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("obstetricia/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_obstetricia: row.id_obstetricia
                });
        ...
        /*if(usuario.admin==0 && row.id_usuario_carga == usuario.id_usuario){
            ...
            if (response.error === "") {
                ...
                if(response.data.length > 1){
                    ...
                    globalsContext.setIdEdit(row.id_obstetricia);
                    navigate("/obstetricia/aprobar-rechazar");
                }
            }
            else
                enqueueSnackbar(response.error, { variant: "error" });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("obstetricia/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_obstetricia: row.id_obstetricia
                });
    ...
    const tableMain = useMemo(() => ({
        title: "Atenciones de Obstetricia",
        idName: "id_obstetricia",
        defaultOrder: "desc",
        headers: [
            ...
            { id: 'dias', label: 'Días de Aten.', dataType: DataType.NUMBER, sortable: true },
            { id: 'horas', label: 'Horas de Aten.', dataType: DataType.NUMBER, sortable: true },
            { id: 'atenTotal', label: 'Atenciones', dataType: DataType.NUMBER, sortable: true },
            { id: 'captaTotal', label: 'Captaciones', dataType: DataType.NUMBER, sortable: true },
            { id: 'controTotal', label: 'Controles', dataType: DataType.NUMBER, sortable: true },
            { id: 'pracTotal', label: 'Prácticas', dataType: DataType.NUMBER, sortable: true },
            { id: 'prevTotal', label: 'Prevención', dataType: DataType.NUMBER, sortable: true },
            { id: 'partos', label: 'Partos', dataType: DataType.NUMBER, sortable: true },
            { id: 'nacRecib', label: 'Rec. Nacidos', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("obstetricia/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
        ...
        rowActions: (row) => {
            ...
            acciones.push({
                label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                    setIdReferenciaVerLogModal(row.id_obstetricia);
                    setReferenciaVerLogModal(Formatter.log_referencia.OBSTETRICIA.id);
                }
            });
    ...
    return (
        <Layout nombrePagina="Atenciones de Obstetricia">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verObstetricia"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
