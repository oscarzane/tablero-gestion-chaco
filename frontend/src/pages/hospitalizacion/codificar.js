...
import FormularioSimpleDialog from '../../components/modal/formularioSimple';
...
function Page() {
    ...
    const [openFormularioSimpleModal, setOpenFormularioSimpleModal] = useState(false);
    const [formSectionsFormularioSimpleModal, setFormSectionsFormularioSimpleModal] = useState([]);
    const [titleFormularioSimpleModal, setTitleFormularioSimpleModal] = useState("");
    const [descriptionFormularioSimpleModal, setDescriptionFormularioSimpleModal] = useState("");
    const [aceptTextFormularioSimpleModal, setAceptTextFormularioSimpleModal] = useState("");
    ...
    const onSubmitFinal = async (values) => {
        ...
        var t_externa_cod = values.externa_cod ? values.externa_cod.codigo : "";
        var t_detalleDiagnosticos = values.detalleDiagnosticos ? values.detalleDiagnosticos : [];
        var t_detalleProcedimientos = values.detalleProcedimientos ? values.detalleProcedimientos : [];

        var t_emb_f_term = values.emb_f_term == null ? null :
        ...
        const response = await postFiles("hospitalizacion/codificar.php", {
            id_usuario: usuario.id_usuario,
            id_hospitalizacion: data.id_hospitalizacion,
            externa_cod: t_externa_cod,
            detalleDiagnosticos: JSON.stringify(t_detalleDiagnosticos),
            detalleProcedimientos: JSON.stringify(t_detalleProcedimientos),
            diagnostico_principal: values.diagnostico_principal,
            tipo_egreso: values.tipo_egreso,
            procedimientos: values.procedimientos,
            externa_prod_por: values.externa_prod_por,
            externa_lugar: values.externa_lugar,
            externa_como: values.externa_como,
            edad: values.edad,
            unidad_edad: values.unidad_edad,
            sexo: values.sexo,
            f_nacimiento: values.f_nacimiento,
            dias_estada: values.dias_estada,
            emb_cant: values.emb_cant,
            emb_f_term: t_emb_f_term,
            comentario: values.comentario,
        });
        if (response.error === "") {
            alertModal.showModal(
                "Informe codificado ok",
                "El informe ha sido codificado con éxito. Puedes salir al menu principal o codificar un nuevo informe:",
                onCodificarPool, "Codificar nuevo",
                onClose, "Salir");

            enqueueSnackbar('Informe codificado ok', { variant: "success" });
        }
        else {
            alertModal.showModal("Error en los datos ingresados", response.error);
    ...
    const guardarParaDespues = () => {
        alertModal.showModal("Saltear informe",
            "El informe será salteado para ser codificado más adelante y se te asignará uno nuevo. ¿Deseas continuar?",
            () => {
                onCodificarPool();
            },
            "Guardar");
    ...
    const submitDevolverParaCorreccion = async (values, { resetForm }) => {
        loadingModal.setOpen(true);

        const response = await postData("hospitalizacion/revisarCodificacion.php", {
            id_usuario: usuario.id_usuario,
            id_hospitalizacion: globalsContext.idEdit,
            resultado_revision: "2",//necesita correccion establecimiento
            comentario: values.observaciones,
            cant_reparos: data.cant_reparos,
        });
        if (response.error === "") {
            enqueueSnackbar("Planilla devuelta ok", { variant: "success" });

            setOpenFormularioSimpleModal(false);
            resetForm({});
    ...
    const devolverParaCorreccion = () => {
        setTitleFormularioSimpleModal("Devolver para corrección");
        setDescriptionFormularioSimpleModal("Podrás devolver esta planilla al establecimiento para que se realicen las correcciones necesarias. Por favor escribe en detalle las observaciones detectadas por corregir.");
        setAceptTextFormularioSimpleModal("Devolver");
        setFormSectionsFormularioSimpleModal([
            {
                name: "",
                fields: [
                    {
        ...
        setOpenFormularioSimpleModal(true);
    };
    ...
    return (
        <Fragment>
            <Layout nombrePagina={"Codificar Hospitalización"}>
                    ...
                    <Grid item xs={12}>
                        <Paper className={classes.paper}>
                            ...
                            <CustomForm
                                ...
                                onSubmit={onSubmit}
                                onCancel={onCancel}
                                submitButtonText={"Finalizar Codificación"}
                ...
                <Grid item xs={12} sm={6} md={4} lg={3} xl={2}>
                    <Button variant="contained" color="error" fullWidth onClick={devolverParaCorreccion} >
            ...
            <FormularioSimpleDialog
                open={openFormularioSimpleModal}
                setOpen={setOpenFormularioSimpleModal}
                formSections={formSectionsFormularioSimpleModal}
                title={titleFormularioSimpleModal}
                description={descriptionFormularioSimpleModal}
                aceptText={aceptTextFormularioSimpleModal}
                onSubmit={submitDevolverParaCorreccion}
            />
