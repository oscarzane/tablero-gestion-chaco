...
function Page() {
    ...
    const onEliminar = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("enfermeria/eliminar.php", {
                    id_usuario: usuario.id_usuario,
                    id_enfermeria: row.id_enfermeria
                });
    ...
    const onReabrirCorreccion = useCallback(async (row) => {
        alertModal.showModal(
            ...
            async function () {
                ...
                const response = await postData("enfermeria/reabrir.php", {
                    id_usuario: usuario.id_usuario,
                    id_enfermeria: row.id_enfermeria
                });
    ...
    const tableMain = useMemo(() => ({
        title: "Prestaciones de Enfermería",
        idName: "id_enfermeria",
        defaultOrder: "desc",
        headers: [
            ...
            { id: 'pac_ate', label: 'Pacientes Atendidos', dataType: DataType.NUMBER, sortable: true },
            { id: 'pre_rea', label: 'Prestaciones Realizadas', dataType: DataType.NUMBER, sortable: true },
        ],
        loadData: async (orderBy, order, page, rowsPerPage, searchText, selectedFilter, pageUpdate, totalUpdate, filterListUpdate) => {
            ...
            const response = await postData("enfermeria/leer.php", {
                id_usuario: usuario.id_usuario,
                orderBy: orderBy,
                order: order,
                page: page,
            ...
            acciones.push(
                { label: "Ver planilla", icon: <PreviewIcon sx={{ color: blue[500] }} />, action: () => onVer(row) },
                {
                    label: "Ver historial", icon: <HistoryRoundedIcon sx={{ color: blue[500] }} />, action: () => {
                        setIdReferenciaVerLogModal(row.id_enfermeria);
                        setReferenciaVerLogModal(Formatter.log_referencia.ENFERMERIA.id);
                    }
                }
            );
    ...
    return (
        <Layout nombrePagina="Prestaciones de Enfermería">
                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <MaterialTable
                            ...
                            tableName={"verEnfermeria"}//se usa para discernir entre cada tabla guardada en globalscontext
                        />
                    </Paper>
                </Grid>
            </Fade>
